...
| `media` | end-to-end media pipeline | `jellyfin`, `plex`, `seerr`, `radarr`, `sonarr`, `whisparr`, `bazarr`, `prowlarr`, `flaresolverr`, `autopulse`, `agregarr`, `configarr`, `decypharr`, `stash` |
...
- `agregarr` is included for Plex collections and recommendations management. Its config lives under `${MEDIA_DATA_ROOT}/agregarr` and the web UI is exposed on port `7171`.
- `agregarr` placeholder support is pre-mounted at `/data/placeholders/movies` -> `/data/movies` and `/data/placeholders/shows` -> `/data/shows`. Add those placeholder folders to Plex, but do not add them to Radarr/Sonarr.
